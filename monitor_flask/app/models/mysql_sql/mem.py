...
class MemModel:
    def __init__(self):
        self.sql_path = os.path.join(sql_path,'mem.sql.toml')
    ...
    def query_hour(self,now_time,next_time):
        return mysql_db.engine.execute(self.mem_hour_query,
                                       now_time=now_time,next_time=next_time).fetchall()


    def query_day(self,now_time,next_time):
        return mysql_db.engine.execute(self.mem_day_query,
                                       now_time=now_time,next_time=next_time).fetchall()


    def query_month(self,now_time,next_time):
        return mysql_db.engine.execute(self.mem_month_query,
                                       now_time=now_time,next_time=next_time).fetchall()